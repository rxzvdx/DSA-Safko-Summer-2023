...
public class Lab07CostAdjacencyMatrixAR 
{
		...
		do 
		{
			...
			switch(iChoice)
			{
				case 1:
					 ...
					 *  3. store the first line in an ArrayList (vertices)...
	...
	private static void populateMatrix()
	{
		...
		try 
		{
			...
			String[] verticesArray = header.split(", "); // Splitting the header into vertices
			vertices = new ArrayList<>(Arrays.asList(verticesArray));
	...
	private static void printMatrix() 
	{
	    if (matrix != null) 
	    {
	        int maxColumnWidth = vertices.stream().mapToInt(String::length).max().orElse(0);

	        // Print header row
	        System.out.printf("%" + (maxColumnWidth + 1) + "s", "");
	        ...
	        {
	            System.out.printf("%" + (maxColumnWidth + 1) + "s", vertices.get(i));
